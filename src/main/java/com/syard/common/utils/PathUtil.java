...
public class PathUtil {
    ...
    public static String getFullPath(String path) throws Exception {
        ...
        return URLDecoder.decode(PathUtil.class.getResource(path).getPath(), "UTF-8");
    }
        ...
        if (pd != null) {
            ...
            if (result != null) {
                ...
                if ("file".equals(result.getProtocol())) {
                    try {
                        if (result.toExternalForm().endsWith(".jar") || result.toExternalForm().endsWith(".zip")) result = new URL("jar:"
                                .concat(result.toExternalForm()).concat("!/").concat(clsAsResource));
